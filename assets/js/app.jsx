...
const AnimatedCircularProgress = ({ label, value, size = 120, strokeWidth = 8 }) => {
  const normalizedValue = Math.min(Math.max(value, 0), 100);
  const radius = Math.max(size / 2 - strokeWidth * 1.5, 0);
  const circumference = 2 * Math.PI * radius;
  const offset = circumference - (normalizedValue / 100) * circumference;
  const style = {
    "--progress-size": `${size}px`,
    "--progress-stroke": strokeWidth,
  };

  return (
    <div className="circular-progress" style={style} role="img" aria-label={`${label} proficiency ${normalizedValue}%`}>
      <svg className="circular-progress__svg" viewBox={`0 0 ${size} ${size}`}>
        <circle className="circular-progress__track" cx={size / 2} cy={size / 2} r={radius} />
        <circle
...
const SkillsBeam = ({ clusters }) => {
  const canAnimate = Boolean(motion);
  const canAnimatePresence = Boolean(motion && AnimatePresence);
  const [activeIndex, setActiveIndex] = useState(0);
  const cardsRef = useRef([]);
  const containerRef = useRef(null);
  const [offsets, setOffsets] = useState([]);

  cardsRef.current = cardsRef.current.slice(0, clusters.length);

  useEffect(() => {
    const updateOffsets = () => {
      if (!containerRef.current) return;
      const newOffsets = cardsRef.current.map((card) => (card ? card.offsetTop + card.offsetHeight / 2 : 0));
  ...
  const indicatorY = offsets[activeIndex] ?? 0;
  const activeCluster = clusters[activeIndex];

  const MotionArticle = canAnimate ? motion.article : "article";
  const MotionPulse = canAnimate ? motion.span : "span";
  const MotionDetail = canAnimate ? motion.div : "div";
  const MotionTag = canAnimate ? motion.span : "span";

  return (
    <div className="skill-beam">
      <div className="skill-beam__rail" aria-hidden="true">
        <span className="skill-beam__line" />
        {canAnimate ? (
          <MotionPulse
            className="skill-beam__pulse"
            animate={{ y: indicatorY }}
            transition={{ type: "spring", stiffness: 320, damping: 28 }}
          />
        ) : (
          <span className="skill-beam__pulse" style={{ transform: `translateY(${indicatorY}px)` }} />
        )}
      </div>
      <div className="skill-beam__cards" ref={containerRef}>
        {clusters.map((cluster, index) => {
          const cardProps = canAnimate
            ? {
                layout: true,
          ...
          return (
            <MotionArticle
              key={cluster.label}
              className={`skill-beam__card ${index === activeIndex ? "is-active" : ""}`}
              ...
              {...cardProps}
            >
              <div className="skill-beam__card-inner">
                <AnimatedCircularProgress
                  ...
                  strokeWidth={10}
                />
                <div className="skill-beam__card-copy">
                  <span className="skill-beam__card-eyebrow">{cluster.category}</span>
                  <h3>{cluster.title}</h3>
                  <p>{cluster.summary}</p>
                </div>
              </div>
              <ul className="skill-beam__list">
      ...
      <div className="skill-beam__detail">
        {canAnimatePresence ? (
          <AnimatePresence mode="wait">
            ...
            >
              <span className="skill-beam__detail-eyebrow">Toolchain focus</span>
              <div className="skill-beam__tags">
...
const AuroraBackground = () => {
  if (!motion || !useMotionValue || !useSpring || !useTransform) return null;
  ...
  const parallaxX = useTransform(springX, (value) => (value - window.innerWidth / 2) / 14);
  const parallaxY = useTransform(springY, (value) => (value - window.innerHeight / 2) / 18);
  ...
  const blobs = [
    { size: 480, color: "var(--aurora-primary)", opacity: 0.65, duration: 22, delay: 0 },
    { size: 420, color: "var(--aurora-secondary)", opacity: 0.55, duration: 28, delay: -6 },
    { size: 360, color: "var(--aurora-tertiary)", opacity: 0.5, duration: 26, delay: -3 },
  ];
...
const skillClusters = [
  {
    ...
    summary:
      "Designing context-aware agents that blend vector search, evals, and prompt tooling to consistently land accurate replies.",
    highlights: [
      "LangChain/LlamaIndex orchestration with streaming guardrails",
      "Hybrid search (ChromaDB, FAISS) tuned via Ragas scorecards",
      "Prompt A/B harnesses with automated regression dashboards",
    ],
    tools: ["LangChain", "LlamaIndex", "OpenAI", "ChromaDB", "FAISS", "Ragas"],
    value: 94,
  },
  {
    label: "LLM Ops",
    category: "Reliability & Observability",
    title: "Governed generative delivery",
    summary:
      "Instrumenting LLM systems with telemetry, guardrails, and release workflows so AI features stay reliable in production.",
    highlights: [
      "Continuous eval loops (Ragas, human QA, prompt diffing)",
      "PII redaction, tool-call idempotency, and safety filters",
      "SLO dashboards with OpenTelemetry traces and feedback loops",
    ],
    tools: ["OpenTelemetry", "Redis", "Promptflow", "Datadog", "Feature Flags", "CI/CD"],
    value: 92,
  },
  {
    label: "MLOps",
    category: "Pipelines & Automation",
    title: "Production-grade ML platforms",
    summary:
      "Automating data to deployment with reproducible pipelines, artifact tracking, and rollout strategies across teams.",
    highlights: [
      "Feature pipelines on Airflow with quality gates and alerts",
      "Model tracking via MLflow/W&B and automated drift pings",
      "Blue/green and canary deploys on SageMaker, ECS, and GKE",
    ],
    tools: ["Airflow", "MLflow", "Weights & Biases", "SageMaker", "GKE", "Terraform"],
    value: 90,
  },
  {
    label: "Computer Vision",
    category: "Perception Systems",
    title: "Real-time vision intelligence",
    summary:
      "Shipping detection and tracking models with optimized data pipelines and accelerated inference at the edge.",
    highlights: [
      "TensorFlow & YOLO training with synthetic augmentation",
      "OpenCV preprocessing and active-learning data loops",
      "CUDA-optimized inference delivered via Docker on AWS",
    ],
    tools: ["TensorFlow", "YOLOv5", "OpenCV", "NVIDIA CUDA", "AWS IoT", "Edge Deploy"],
    value: 88,
  },
  {
    label: "Cloud & DevOps",
    ...
    summary:
      "Crafting resilient infrastructure with automation-first practices, observability, and developer experience in mind.",
    highlights: [
      "IaC with Terraform spanning VPC, networking, and secrets",
      "GitHub Actions + CodePipeline CI/CD with policy checks",
      "Event-driven services via FastAPI, Kafka, and gRPC APIs",
    ],
    tools: ["AWS", "GCP", "Terraform", "GitHub Actions", "Kafka", "FastAPI"],
    value: 86,
  },
];
  ...
  <section className="section" id="skills">
    <BlurFade delay={100}>
      <header className="section__header">
        ...
        <h2>Interactive map of the stacks I use to launch trustworthy AI.</h2>
        <p>
          Hover a competency to send a beam of light across the stack and surface the toolchain powering each delivery lane.
        </p>
      </header>
    </BlurFade>
    <BlurFade delay={220}>
      <SkillsBeam clusters={skillClusters} />
    </BlurFade>
    <BlurFade delay={360}>
